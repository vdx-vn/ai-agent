...
1. "Test this stock customization against the current project database."
   - Expected: use `odoo-test` as primary skill and compose with `odoo-local-test-harness` when execution depends on `ODOO_TEST_BASE_CMD`, config inspection, existing-db resolution, or disposable DB/filestore cleanup.
...
- Why this skill wins: The user wants current-change validation evidence, not framework education or review findings. `odoo-test` should win over `odoo-testing-reference` and `odoo-review` while composing with `odoo-local-test-harness` when execution depends on `ODOO_TEST_BASE_CMD`, config inspection, existing-db resolution, or disposable DB/filestore cleanup.
