...
2. "This repo is on Odoo 17.0, but the local command differs from my 18.0 project."
   - Expected: use `odoo-local-test-harness` as primary skill.
3. "Clean the temporary Odoo test database and filestore after this run."
   - Expected: use `odoo-local-test-harness` as primary skill.
...
## Tie-breaker
- Prompt: "Run the local Odoo command for this project with `--test-tags /sale` on a disposable db."
