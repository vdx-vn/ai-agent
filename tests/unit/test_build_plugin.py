...
class BuildPluginTests(unittest.TestCase):
    def test_build_marketplace_creates_runtime_subset_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("claude plugin install odoo-skills-v19@odoo-skills-v19 --scope local", runtime_readme)
            ...
            self.assertLess(
                ...
                runtime_readme.index("claude plugin install odoo-skills-v19@odoo-skills-v19 --scope local"),
            )
            self.assertLess(
