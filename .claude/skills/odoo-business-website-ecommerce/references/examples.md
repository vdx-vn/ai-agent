...
2. "How should compute dependencies work?"
   - Expected: do not use `odoo-business-website-ecommerce` as primary skill.

## Tie-breaker
- Prompt: "What happens when a shopper checks out with an out-of-stock item?"
- Why this skill wins: The entrypoint is cart and checkout behavior, so `odoo-business-website-ecommerce` should win over `odoo-business-sales`.
